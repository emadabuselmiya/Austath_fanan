...
class UserController extends Controller
{
    ...
    {
        $this->authorizeForUser(auth('admin')->user(), 'viewAny', Admin::class);
        ...
        if ($request->ajax()) {
            ...
            foreach ($records as $record) {
                $delete = '';
                $edit = '';
                if (auth()->guard('admin')->user()->can('delete', \App\Models\Admin::class)) {
                ...
                if (auth()->guard('admin')->user()->can('update', \App\Models\Admin::class)) {
...
//                    "last_activity" => Carbon::parse($record->last_activity)->format('Y-m-d g:i A'),
    ...
    public function store(Request $request)
    {
        ...
        if (!auth()->guard('admin')->user()->can('create', Admin::class)) {
            return response()->json([
                'status' => false,
                'code' => 'update',
                'message' => translate('You still have the authority to perform this operation.')], 200);
        }
