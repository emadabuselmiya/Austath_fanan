                        ...
                        <div class="row mt-5">
                            ...
                            <div class="col-md-4">
                                ...
                                <select id="status" name="status" required class="form-control select2"
                                    ...
                                    <option value="1">{{translate('ارسال فوري')}}</option>
                                    <option value="2">{{translate('ارسال مجدول')}}</option>

                                </select>

                            </div>

                            <div class="col-md-4" id="schedule_at_div" style="display: none">
                            ...
                            <div class="col-md-4">
                                ...
                                <select id="target" name="target[]" required onchange="notify_target_change(this.value)"
                                    ...
                                    <option value="customer">{{translate('الزبائن')}}</option>
                                    <option value="guest">{{translate('الزوار')}}</option>
                                    <option value="deliveryman">{{translate('كباتن التوصيل')}}</option>
                                    {{--                                    <option value="restaurant">{{translate('messages.restaurant')}}</option>--}}
                                </select>

                            </div>
                            <div class="col-md-4" id="platform" style="display:none;">
                                <label for="platform" class="form-label">{{ translate('منصة') }}<span
                                        class="text-danger">*</span></label>

                                <select id="platform" name="platform" class="form-control select2">
                                    <option value="all">{{translate('كل منصات')}}</option>
                                    <option value="ios">{{translate('ios')}}</option>
                                    <option value="android">{{translate('اندرويد')}}</option>
                                </select>
                            ...
                            <div class="col-md-4" id="type" style="display:none;">
                                ...
                                <select id="type" name="type" required onchange="notify_type_change(this.value)"
                                    ...
                                    <option value="store">{{translate('المتجر')}}</option>
                                    <option value="market">{{translate('ماركت')}}</option>
                                    <option value="product">{{translate('المنتج')}}</option>
                                    <option value="category">{{translate('الفئات')}}</option>
                                    <option value="url">{{translate('url')}}</option>
                                    <option value="invite_friend">{{translate('صفحة دعوة صديق')}}</option>
                                    <option value="points">{{translate('صفحة نقاطي')}}</option>
{{--                                    <option value="wallet">{{translate('صفحة المحفظة')}}</option>--}}
                                </select>
                            </div>

                            <div class="col-md-4" id="type_dm" style="display:none;">
                                <label for="type" class="form-label">{{ translate('الهدف') }}</label>

                                <select id="type" name="type" required onchange="notify_type_change(this.value)"
                                        class="form-control select2">
                                    <option disabled selected>{{translate('اختار الهدف')}}</option>
                                    <option value="all">{{translate('كل الكباتن')}}</option>
                                    <option value="dm_online">{{translate('الكباتن المتصلين')}}</option>
                                    <option value="dm_offline">{{translate('الكباتن الغير متصلين')}}</option>
                                </select>

                            </div>

                            <div class="col-md-4" id="store" style="display:none;">
                            ...
                            <div class="col-md-4" id="product" style="display:none;">
                                <label for="target" class="form-label">{{ translate('المنتج') }}<span
                                        class="text-danger">*</span></label>

                                <select name="product" class="js-product-ajax form-control"
                                        title="Select product">
                                    <option selected disabled>{{ translate('اختار المنتج') }}</option>
                                </select>
                            </div>

                            <div class="col-md-4" id="category" style="display:none;">
                                ...
                                <select name="category" class="select2 form-control"
                                    ...
                                    <option value="-2">{{ translate('عروض خاصة') }}</option>
                            ...
                            <div class="col-md-4" id="url" style="display:none;">
...
@section('js')
    ...
    <script>
        ...
        $('.js-product-ajax').select2({
            ajax: {
                url: '{{route('admin.products.get_products')}}?ispPages=1',
